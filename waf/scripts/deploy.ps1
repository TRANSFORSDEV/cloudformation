...
--parameter-overrides `
  ...
  MasterElbStack=$COID-$ASSETID-$APID-$SID-ST-$STACK_NUMBER-MASTER-ELB `
  MasterApiGatewayStack="" `
  WafName=$WAF_APP_NAME `
  DefaultAction=$WAF_APP_DEFAULT_ACTION `
  A1Injection=$A1_INJECTION `
  A2BrokenAuthentication=$A2_BROKEN_AUTHENTICATION `
  A3CrossSiteScripting=$A3_CROSS_SITE_SCRIPTING `
  ...
  HttpFloodProtection=$HTTP_FLOOD_PROTECTION `
