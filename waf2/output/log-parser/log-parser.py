...
def get_outstanding_requesters(log, bucket_name, key_name, log_type):
    ...
    try:
        ...
        remove(local_file_path)
        # print("json.dumps(counter):"+json.dumps(counter))
        ...
        # threshold = 'errorThreshold' if log_type == 'waf' else "requestThreshold"
        utc_now_timestamp_str = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z%z")
...
# For Token on the same device
def get_outstanding_requesters_for_token(log, bucket_name, key_name, log_type):
    log.debug('[get_outstanding_requesters] Start')
    ...
    try:
        # --------------------------------------------------------------------------------------------------------------
        log.info("[get_outstanding_requesters_for_token] \tDownload file from S3")
        # --------------------------------------------------------------------------------------------------------------
        local_file_path = '/tmp/' + key_name.split('/')[-1]
        s3 = boto3.client('s3')
        s3.download_file(bucket_name, key_name, local_file_path)
        ...
        # --------------------------------------------------------------------------------------------------------------
        log.info("[get_outstanding_requesters_for_token] \tRead file content")
        # --------------------------------------------------------------------------------------------------------------
        with gzip.open(local_file_path, 'r') as content:
        # with open(local_file_path, 'r') as content:
            for line in content:
                try:
                    ...
                    if log_type == 'waf':
                        ...
                        request_key = datetime.datetime.fromtimestamp(int(line_data['timestamp']) / 1000.0).isoformat(
                            sep='T', timespec='minutes')
                        headers = line_data['httpRequest']['headers']
                        for header in headers:
                            if header['name'] == 'Authorization':
                                token = header['value']
                                break
                        request_key += ' ' + token
                        ipInvoke = line_data['httpRequest']['clientIp']
                        uri = urlparse(line_data['httpRequest']['uri']).path
                    ...
                    if 'ignoredSufixes' in config['general'] and uri.endswith(
                            tuple(config['general']['ignoredSufixes'])):
                        log.debug(
                            "[get_outstanding_requesters_for_token] \t\tSkipping line %s. Included in ignoredSufixes." % line)
                        continue

                    if return_code_index == None or line_data[return_code_index] in config['general']['errorCodes']:
                        if request_key in counter['general'].keys():
                            if ipInvoke not in counter['general'][request_key]['ipsInvoke']:
                                counter['general'][request_key]['ipsInvoke'].append(ipInvoke)
                            counter['general'][request_key]['count'] += 1
                        else:
                            counter['general'][request_key] = {}
                            counter['general'][request_key]['ipsInvoke'] = [ipInvoke]
                            counter['general'][request_key]['count'] = 1
                    ...
                    log.error("[get_outstanding_requesters_for_token] \t\tError to process line: %s" % line)
        remove(local_file_path)

        # --------------------------------------------------------------------------------------------------------------
        log.info("[get_outstanding_requesters_for_token] \tKeep only outstanding requesters")
        # --------------------------------------------------------------------------------------------------------------
        threshold = 'requestThreshold' if log_type == 'waf' else "errorThreshold"
        # threshold = 'errorThreshold' if log_type == 'waf' else "requestThreshold"
        utc_now_timestamp_str = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z%z")
        for k, req_key in counter['general'].items():
            try:
                k = k.split(' ')[-1]
                num_reqs = req_key['count']
                for ipInvoke in req_key['ipsInvoke']:
                    if num_reqs >= config['general'][threshold]:
                        if ipInvoke not in outstanding_requesters['general'].keys() or num_reqs > \
                                outstanding_requesters['general'][ipInvoke]['max_counter_per_min']:
                            outstanding_requesters['general'][ipInvoke] = {
                                'max_counter_per_min': num_reqs,
                                'updated_at': utc_now_timestamp_str
                            }
            except Exception as e:
                log.error(
                    "[get_outstanding_requesters_for_token] \t\tError to process outstanding requester: %s" % k)
        ...
        for uri in counter['uriList'].keys():
            for k, num_reqs in counter['uriList'][uri].items():
                try:
                    ...
                    log.error(
                        "[get_outstanding_requesters_for_token] \t\tError to process outstanding requester: (%s) %s" % (uri, k))

    except Exception as e:
        log.error("[get_outstanding_requesters_for_token] \tError to read input file")
        log.error(e)

    log.debug('[get_outstanding_requesters_for_token] End')
    return outstanding_requesters


def get_outstanding_requesters_for_device(log, bucket_name, key_name, log_type):
    log.debug('[get_outstanding_requesters_for_device] Start')
    ...
    try:
        # --------------------------------------------------------------------------------------------------------------
        log.info("[get_outstanding_requesters_for_device] \tDownload file from S3")
        # --------------------------------------------------------------------------------------------------------------
        local_file_path = '/tmp/' + key_name.split('/')[-1]
        s3 = boto3.client('s3')
        s3.download_file(bucket_name, key_name, local_file_path)
        ...
        # --------------------------------------------------------------------------------------------------------------
        log.info("[get_outstanding_requesters_for_device] \tRead file content")
        # --------------------------------------------------------------------------------------------------------------
        with gzip.open(local_file_path, 'r') as content:
        # with open(local_file_path, 'r') as content:
            for line in content:
                try:
                    ...
                    if 'ignoredSufixes' in config['general'] and uri.endswith(
                            tuple(config['general']['ignoredSufixes'])):
                        log.debug(
                            "[get_outstanding_requesters_for_device] \t\tSkipping line %s. Included in ignoredSufixes." % line)
                        continue
                    ...
                    log.error("[get_outstanding_requesters_for_device] \t\tError to process line: %s" % line)
        remove(local_file_path)

        # --------------------------------------------------------------------------------------------------------------
        log.info("[get_outstanding_requesters_for_device] \tKeep only outstanding requesters")
        # --------------------------------------------------------------------------------------------------------------
        threshold = 'requestThreshold' if log_type == 'waf' else "errorThreshold"
        # threshold = 'errorThreshold' if log_type == 'waf' else "requestThreshold"
        utc_now_timestamp_str = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z%z")
        for k, req_key in counter['general'].items():
            try:
                k = k.split(' ')[-1]
                num_reqs = req_key['count']
                if len(req_key['ipsInvoke'])>1:
                    for ipInvoke in req_key['ipsInvoke']:
                        if ipInvoke not in outstanding_requesters['general'].keys():
                            outstanding_requesters['general'][ipInvoke] = {
                                'max_counter_per_min': num_reqs,
                                'updated_at': utc_now_timestamp_str
                            }
                ...
                log.error(
                    "[get_outstanding_requesters_for_device] \t\tError to process outstanding requester: %s" % k)
        ...
        for uri in counter['uriList'].keys():
            for k, num_reqs in counter['uriList'][uri].items():
                try:
                    ...
                    log.error(
                        "[get_outstanding_requesters_for_device] \t\tError to process outstanding requester: (%s) %s" % (uri, k))

    except Exception as e:
        log.error("[get_outstanding_requesters_for_device] \tError to read input file")
        log.error(e)

    log.debug('[get_outstanding_requesters_for_device] End')
    return outstanding_requesters
...
def merge_outstanding_requesters_for_device(log, bucket_name, key_name, log_type, output_key_name, outstanding_requesters):
    log.debug('[merge_outstanding_requesters] Start')
    ...
    try:
        ...
        log.info('[merge_outstanding_requesters_for_device] No file to be merged.')
        need_update = True
        return outstanding_requesters, need_update
    ...
    # --------------------------------------------------------------------------------------------------------------
    log.info("[merge_outstanding_requesters_for_device] \tDownload current blocked IPs")
    # --------------------------------------------------------------------------------------------------------------
    local_file_path = '/tmp/' + key_name.split('/')[-1] + '_REMOTE.json'
    s3.download_file(bucket_name, output_key_name, local_file_path)

    # ----------------------------------------------------------------------------------------------------------
    log.info("[merge_outstanding_requesters_for_device] \tProcess outstanding requesters files")
    # ----------------------------------------------------------------------------------------------------------
    remote_outstanding_requesters = {
        'general': {},
        'uriList': {}
        ...
        if 'general' in remote_outstanding_requesters:
            for k, v in remote_outstanding_requesters['general'].items():
                try:
                    if k in outstanding_requesters['general'].keys():
                        log.info(
                            "[merge_outstanding_requesters_for_device] \t\tUpdating general data of BLOCK %s rule" % k)
                        outstanding_requesters['general'][k]['updated_at'] = utc_now_timestamp_str
                        if v['max_counter_per_min'] > outstanding_requesters['general'][k]['max_counter_per_min']:
                            outstanding_requesters['general'][k]['max_counter_per_min'] = v['max_counter_per_min']
                        ...
                        # el
                        if total_diff_min < config['general']['blockPeriod']:
                            log.debug("[merge_outstanding_requesters_for_device] \t\tKeeping %s in general" % k)
                            outstanding_requesters['general'][k] = v

                        else:
                            force_update = True
                            log.info("[merge_outstanding_requesters_for_device] \t\t%s expired in general" % k)

                except Exception:
                    log.error("[merge_outstanding_requesters_for_device] \tError merging general %s rule" % k)
    except Exception:
        log.error('[merge_outstanding_requesters_for_device] Failed to process general group.')
    ...
    try:
        if 'uriList' in remote_outstanding_requesters:
            if 'uriList' not in config or len(config['uriList']) == 0:
                force_update = True
                log.info(
                    "[merge_outstanding_requesters_for_device] \t\tCurrent config file does not contain uriList anymore")
            else:
                for uri in remote_outstanding_requesters['uriList'].keys():
                    if 'ignoredSufixes' in config['general'] and uri.endswith(
                            tuple(config['general']['ignoredSufixes'])):
                        force_update = True
                        log.info(
                            "[merge_outstanding_requesters_for_device] \t\t%s is in current ignored sufixes list." % uri)
                        continue
                    ...
                    for k, v in remote_outstanding_requesters['uriList'][uri].items():
                        try:
                            if uri in outstanding_requesters['uriList'].keys() and k in \
                                    outstanding_requesters['uriList'][uri].keys():
                                log.info(
                                    "[merge_outstanding_requesters_for_device] \t\tUpdating uriList (%s) data of BLOCK %s rule" % (
                                    uri, k))
                                outstanding_requesters['uriList'][uri][k]['updated_at'] = utc_now_timestamp_str
                                if v['max_counter_per_min'] > outstanding_requesters['uriList'][uri][k][
                                    'max_counter_per_min']:
                                ...
                                if v['max_counter_per_min'] < config['uriList'][uri][threshold]:
                                    force_update = True
                                    log.info(
                                        "[merge_outstanding_requesters_for_device] \t\t%s is bellow the current uriList (%s) threshold" % (
                                        k, uri))

                                elif total_diff_min < config['general']['blockPeriod']:
                                    log.debug(
                                        "[merge_outstanding_requesters_for_device] \t\tKeeping %s in uriList (%s)" % (k, uri))
                                    ...
                                    force_update = True
                                    log.info(
                                        "[merge_outstanding_requesters_for_device] \t\t%s expired in uriList (%s)" % (k, uri))

                        except Exception:
                            log.error(
                                "[merge_outstanding_requesters_for_device] \tError merging uriList (%s) %s rule" % (uri, k))
    except Exception:
        log.error('[merge_outstanding_requesters_for_device] Failed to process uriList group.')

    print(json.dumps({
        'force_update': force_update,
        'last_update_age': last_update_age,
        'int(os.getenv(MAX_AGE_TO_UPDATE))': int(os.getenv('MAX_AGE_TO_UPDATE')),
        'len(outstanding_requesters[general])': len(outstanding_requesters['general']),
        'len(outstanding_requesters[uriList])': len(outstanding_requesters['uriList'])
    }))
    need_update = (force_update or
                   last_update_age > int(os.getenv('MAX_AGE_TO_UPDATE')) or
                   len(outstanding_requesters['general']) > 0 or
    ...
    log.debug('[merge_outstanding_requesters_for_device] End')
    return outstanding_requesters, need_update
...
def process_log_file(log, bucket_name, key_name, conf_filename, output_filename, log_type, ip_set_type):
    ...
    list_outstanding_requesters = [{'output_filename':output_filename,'outstanding_requesters':get_outstanding_requesters(log, bucket_name, key_name, log_type)}]
    if str(os.getenv('COUNT_TOKEN').upper()) == 'YES':
        list_outstanding_requesters.append({'output_filename':output_filename.replace('.json','_count_token.json'),'outstanding_requesters':get_outstanding_requesters_for_token(log, bucket_name, key_name, log_type)})

    if str(os.getenv('CHANGE_DEVICE_TOKEN').upper()) == 'YES':
        list_outstanding_requesters.append({'output_filename':output_filename.replace('.json','_change_device_token.json'),'outstanding_requesters':get_outstanding_requesters_for_device(log, bucket_name, key_name, log_type)})

    print(json.dumps(list_outstanding_requesters),"json.dumps(list_outstanding_requesters)")
    for outstanding_requester in list_outstanding_requesters:
        if '_change_device_token.json' in outstanding_requester['output_filename']:
            outstanding_requesters, need_update = merge_outstanding_requesters_for_device(log, bucket_name, key_name, log_type, outstanding_requester['output_filename'],
                                                                           outstanding_requester['outstanding_requesters'])
        else:
            outstanding_requesters, need_update = merge_outstanding_requesters(log, bucket_name, key_name, log_type, outstanding_requester['output_filename'],
                                                                           outstanding_requester['outstanding_requesters'])
        print(json.dumps(outstanding_requesters))
        print(str(need_update))
        if need_update:
            # ----------------------------------------------------------------------------------------------------------
            log.info("[process_log_file] \tUpdate new blocked requesters list to S3")
            # ----------------------------------------------------------------------------------------------------------
            write_output(log, bucket_name, key_name, outstanding_requester['output_filename'], outstanding_requesters)
